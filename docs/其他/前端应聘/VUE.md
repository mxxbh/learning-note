...
- 响应式原理的变化：（最根本的变化）
  - Vue 2 使用 `Object.defineProperty` 实现，无法检测对象属性的动态添加/删除，无法直接监听数组索引和length的变化。
  - Vue 3 使用 `Proxy` 实现，支持更多类型的变化监听，性能更好。
- API 风格的变化：
  - Vue 2 主要使用 Options API，组件逻辑分散在不同选项中。
  - Vue 3 引入 Composition API，提供更灵活的组件逻辑组织方式，适合大型项目和复杂组件。
- 生命周期的变化：
  - Vue 3 的生命周期钩子名称有所调整，如 `beforeDestroy` 改为 `beforeUnmount`，`destroyed` 改为 `unmounted`。
  - Vue3 中 setup 的执行时机在 beforeCreate 和 created 之间，因此这两个生命周期在 Composition API 中基本被 setup() 替代。
  - 新增调试生命周期：onRenderTracked、onRenderTriggered。
- 组件挂载根节点的变化：
  - Vue 2 只能挂载到一个元素节点上。
  - Vue 3 支持 Fragment，可以挂载到多个根节点上，减少无用标签。
- 性能优化：
  - Vue 3 更快、更小，支持 Tree-shaking。
- TypeScript 支持：
  - Vue 3 原生支持 TypeScript，类型推断更好。