...
- localStorage：持久化存储，关闭浏览器后数据不丢失，同域下所有页面共享，容量约 5MB。
- sessionStorage：会话级存储，关闭页面或标签页后数据清除，每个标签页独立，容量约 5MB。
- Cookie：每次请求自动携带到服务端，容量约 4KB，可设置过期时间，有 HttpOnly、Secure、SameSite 等属性。
- IndexedDB：客户端关系型数据库，可存储大量结构化数据，容量远超 localStorage，支持事务和索引。

## 如何解决数字精度丢失问题？

JavaScript 使用 IEEE 754 双精度浮点数，0.1 + 0.2 !== 0.3 是经典问题。常见解决方式：

- 转为整数运算，先乘以倍数转为整数计算后再除回，如 `(0.1 * 10 + 0.2 * 10) / 10`。
- toFixed + parseFloat，`parseFloat((0.1 + 0.2).toFixed(10))`，toFixed 会返回字符串再转回数字。
- 使用精度库，如 decimal.js、big.js，适合金融等对精度要求高的场景。
- `Number.EPSILON` 比较，判断两个浮点数是否相等时用 `Math.abs(a - b) < Number.EPSILON`。

## 如何获取安全的 undefined 值？

`undefined` 可以被当作变量来使用和赋值，所以直接给变量赋值 `undefined` 可能会出现问题。  
表达式 `void 0` 返回结果是 `undefined`（void 并不改变表达式的结果，只是让表达式不返回值），因此可以用 `void 0` 来获得 `undefined`。

## 如何实现自执行函数 IIFE？

核心思路是让函数变成表达式而非声明，然后立即调用。常见写法：
...
(() => { /* ... */ })();        // 箭头函数
!function() { /* ... */ }();    // 一元运算符前缀
void function() { /* ... */ }(); // void 前缀
```

IIFE 的主要用途是创建独立作用域，避免污染全局命名空间。
