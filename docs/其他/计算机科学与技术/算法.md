...
对 n 个元素的序列进行堆排序：
1、先将其建成堆，以根节点与第 n 个节点交换，再调整前 n-1 个节点成为堆。
2、再以根节点与第 n-1 个节点交换，重复上述操作，直到整个序列有序。
...
3、删除边后，如果这条边另一个节点的入度变成 0，则将该节点加入集合中。
4、循环这个过程，直到把所有顶点都遍历完成。
5、如果存放入度为 0 的节点的集合为空，但此时还有未被遍历的边，则说明图中至少存在一个回路。
